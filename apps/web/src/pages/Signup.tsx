...
export default function Signup() {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // the account, so this works for both new and returning users.
      ...
      setError(msg === 'Something went wrong.' ? "Google sign-in failed. Try again or use email." : msg);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="w-full md:w-1/2 bg-[#F3F6F8] p-4 sm:p-6 md:p-7 flex items-center justify-center order-1 md:order-1 overflow-hidden">
          <motion.div
          ...
          >
            <img 
              ...
              className="w-full max-w-[460px] sm:max-w-[500px] md:max-w-[540px] max-h-[420px] md:max-h-[440px] object-contain scale-x-[-1] drop-shadow-sm"
        ...
        <div className="w-full md:w-1/2 p-6 sm:p-8 md:p-10 flex flex-col justify-center items-center order-2 md:order-2">
          <div className="w-full max-w-[350px] flex flex-col items-center">
            ...
            <p className="mt-4 text-xs sm:text-sm text-gray-500">
              Already have an account? <Link to="/login" className="text-[#6B1302] font-semibold hover:underline">Login</Link>
            </p>
          </div>
        </div>
      </motion.div>
